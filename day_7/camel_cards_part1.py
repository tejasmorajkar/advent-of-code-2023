...
def get_rank(hand: str) -> tuple:
    ...
    return classify_hand_type(hand), "".join([letter_mapping.get(char, char) for char in hand])
...
def test_camel_cards_part1():
    with (open("test.txt", "r") as input_file):
        input_text = input_file.read()
        plays = parse_input(input_text)
        actual_result = calculate_total_winnings(plays)
        assert actual_result == 6440, (f"Test for calculating total winnings failed!!! "
                                       f"Expected 6440, but got {actual_result}")


def camel_cards_part1_main():
...
if __name__ == "__main__":
    test_camel_cards_part1()
    camel_cards_part1_main()
